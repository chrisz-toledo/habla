...
def test_fuzz_normalizes_url():
    from hado.cybersec.fuzzer import fuzz, _normalize_url
...
def test_fuzz_default_wordlist():
    from hado.cybersec.fuzzer import fuzz, _DEFAULT_WORDLIST
...
def test_dns_records_returns_dict():
    from hado.cybersec.recon import dns_records
...
def test_whois_lookup_returns_dict_with_domain():
    from hado.cybersec.recon import whois_lookup
...
def test_email_harvest_returns_list():
    from hado.cybersec.recon import email_harvest
...
def test_enumera_import_included():
    from hado import compile_to_source
    out = compile_to_source("enumera directories en objetivo")
    assert "from hado.cybersec.fuzzer import fuzz as _hado_fuzz" in out


def test_enumera_assignment():
    from hado import compile_to_source
    out = compile_to_source("paths = enumera directories en objetivo")
    assert "paths = _hado_fuzz" in out
...
def test_busca_subdomains_compiles():
    ...
    assert "_hado_find_subdomains" in out
...
def test_enumera_produces_correct_ast():
    ...
    from hado.ast_nodes import CyberEnumerate, ExpressionStatement
